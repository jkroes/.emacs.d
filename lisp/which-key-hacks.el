...
; TODO: Investigate treemacs--helpful-hydra/heads' structure sometime, so you can reconcile these functions. Checkout
; the definition of treemacs-helpful-hydra, which defines the hydra, and compare hydra-window/heads to
; treemacs--helpful-hydra/heads.
(defun my/defhydra2 (name)
  "Needed for treemacs--helpful-hydra. The only difference is in the definition of h-cmd. For some reason the variable
treemacs--helpful-hydra/heads is structured differently from my hydras. It looks the second element of each list is a
  function, but type-of reveals that it's a cons cell."
  (let* ((prefix-sans-slash (symbol-name name))
     (prefix (concat prefix-sans-slash "/")))
    ;; Replacements for which-key descriptions of hydra heads. wk does at most a single
    ;; replacement, unless which-key-allow-multiple-replacements is non-nil. In lieu of setting
    ...
    ;; Retrieving/modifying docstrings in case toggled in which-key
    (dolist (h (symbol-value (intern (concat prefix "heads"))))
      (let* ((h-cmd (nth 1 (nth 1 h))) ;; The original command in the hydradef
         ;; hydra renames commands in several possible ways, depending on :color
         (visible-cmd1 (intern (concat prefix (symbol-name h-cmd))))
         (visible-cmd2 (intern (concat (symbol-name visible-cmd1) "-and-exit"))))
    ;; NOTE: To retrieve the original docstring defined in the function, you must
