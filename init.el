...
;; See '48.2 Package Installation' (emacs) and '41.1 Packaging Basics' (elisp)
;; NOTE: This is different from loading a Lisp library.
;;  The term loading apparently means modifying `load-path' and loading a package's autoloads;
;;  this means activation is full loading/evaluation of a package. But the terms don't seem
;;  to be used consistently. Modification of load-path can be disabled via (setq package-
;;  enable-at-startup nil) and removing any (package-initialize) from init file, or by
;;  running (package-initialize t) in init file. See their documentation for an
;;  illustration of this inconsistency.
...
;; (dolist (pkg '(quelpa help-fns+ smex flx hydra))
